...
M9A 提供了对 Conda 虚拟环境的支持，相比标准 venv，Conda 能更好地管理复杂依赖和不同 Python 版本。
...
M9A 默认已启用 Conda 环境支持。只需确保系统中已安装 Anaconda 或 Miniconda，然后直接运行程序即可自动检测并使用 Conda。

**手动配置方式**：

1. 确保已安装 Anaconda 或 Miniconda
2. 使用项目根目录中的一键脚本：
   - Windows: 双击运行 `setup_and_run_conda.bat`
   - Linux/macOS: 在终端中执行 `chmod +x setup_and_run_conda.sh && ./setup_and_run_conda.sh`

这些脚本会：
- 检查并创建名为 `m9a_env` 的 Conda 环境
- 安装项目所需的所有依赖
- 设置正确的环境变量
- 启动 M9A
...
如需临时使用标准 venv 而非 Conda，可设置环境变量：
```bash
# Windows (cmd)
